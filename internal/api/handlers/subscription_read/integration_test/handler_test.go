...
func TestHandler_Handle(t *testing.T) {
	...
	storage := get_subscription.NewStorage(db)
	usecase := get_subscription.NewUsecase(storage)
	handler := subscription_read.NewHandler(logger, usecase)

	req, err := http.NewRequest("GET", fmt.Sprintf("/subscriptions/%d", 2), nil)
	require.NoError(t, err)

	req.Header.Add("Content-Type", "application/json")

	router := gin.Default()
	router.GET("/subscriptions/:subscriptionId", handler.Handle)
